...
# TODO: Migrate to requests.py
async def reaction_requestAnswered(source: Message | Reaction, yes: bool, *, requests: Requests, obj: Group | Vectorstore, bot: Bot) -> None:
	"""Handles a response to a request."""
	sourceMessage = source.message if isinstance(source, Reaction) else source
	record = requests[sourceMessage]
	if yes:
		# If the request was for a user to join permitting:
		if isinstance(obj, Group):
			if record is None: return
			if obj.add(record["recipientID"]) < 1:
				await Output.replyWithinCharacterLimit(sourceMessage, "Failed to add user to group.")
			...
			if record is None:
				if (count := obj.add(sourceMessage.content, sources=sourceMessage.jump_url)) < 1:
					await Output.replyWithinCharacterLimit(sourceMessage, "Failed to add [count] message[plural] to the vectorstore.".replace("[count]", str(1 - count)).replace("[plural]", getLanguagePlural(LANGUAGE, 1 - count)))
					return
				if bot.user is not None:
					await sourceMessage.add_reaction("✅")
					await sourceMessage.remove_reaction("✅", bot.user)
				return
			# Otherwise it was an individual request that was accepted
				...
				await Output.replyWithinCharacterLimit(sourceMessage, "Failed to add [count] message[plural] to the vectorstore.".replace("[count]", str(len(record["desiredMessages"]) - count)).replace("[plural]", getLanguagePlural(LANGUAGE, len(record["desiredMessages"]) - count)))
	# Delete record of request
	if not requests.remove(sourceMessage):
		await Output.replyWithinCharacterLimit(sourceMessage, "Failed to delete request record.")
	...
	if isinstance(obj, Group) or record is not None:
		...
		await sourceMessage.delete(delay=1*yes)